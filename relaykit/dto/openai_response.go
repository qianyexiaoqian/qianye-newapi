...
// ReconcileReasoningTokens 把上游"没有算进 completion_tokens"的推理 token 补回去。
//
// ═══════════════════════ 它修的是一个资损口子 ═══════════════════════
//
// 部分 OpenAI 兼容上游把 reasoning/thinking 单独计入 total_tokens,而**不**并进
// completion_tokens:
//
...
//	                     8 + 42 + 471 = 521
//
// 网关此前原样透传,并在计费处按 prompt+completion 记账 —— 于是站点为 521 个 token
// 向上游付费,只按 50 个向用户收钱(实测一次真实调用:应收 772 quota,实收 65)。
// 思考型模型上这个缺口是数量级的。
//
// ═══════════════════════ 为什么是 max(completion, total-prompt) ═══════════════════════
//
// 直接 `completion += reasoning` 会在**已经把 reasoning 算进 completion** 的上游
// (标准 OpenAI 就是这么做的)上重复计一次,把少收变成多收 —— 那比现在更糟。
//
// 取 max 对两类上游都正确:
//
//	标准上游  total = prompt + completion        → total-prompt = completion,恒等,零变化
//	本站上游  total = prompt + completion + reas → total-prompt = completion+reas,补齐
//
// 它同时还兜住"上游只给了 total、completion 留空"这种残缺响应。
//
// ═══════════════════════ 边界 ═══════════════════════
//
// total 为 0(上游没给)或 total < prompt(明显错乱)时**一个字节都不改** ——
// 那种响应本来就不可信,拿它去调高收费是拿用户的钱赌上游的正确性。
//
// 同仓另外三个适配器早就在做同样的对账(relaykit 的 gemini 原生转换直接写
// CandidatesTokenCount+ThoughtsTokenCount,xai 与 baidu 都是 total-prompt),
// 只有 OpenAI 兼容这一条路没有 —— 本方法就是把它补齐,不是新增口径。
func (u *Usage) ReconcileReasoningTokens() {
	if u == nil || u.TotalTokens <= 0 {
		return
	}
	if u.TotalTokens < u.PromptTokens {
		return
	}
	if derived := u.TotalTokens - u.PromptTokens; derived > u.CompletionTokens {
		u.CompletionTokens = derived
	}
}

...
// that an oversized tiered settlement clamps to common.MaxQuota instead of
...
func TestComputeTieredQuota_ClampOnOverflow(t *testing.T) {
	...
	assert.Equal(t, common.MaxQuota, result.ActualQuotaAfterGroup, "oversized quota must clamp to MaxQuota, never wrap negative")

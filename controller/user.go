...
func UpdateUser(c *gin.Context) {
	...
	// 任何人记得那条订阅。项目方拍板:改组即清记录。
	//
	// ⚠ **保存前的提醒目前还没做。** 早先这里写着"界面上另有保存前的提醒",
	// 那是一句假话:model.CountUserGroupSubscriptions 至今零调用方,用户编辑页
	// 的分组下拉是一个裸 Select,管理员是在毫无提示的情况下触发这次不可逆摘除的
	// (三个快照字段清空后无法还原)。审计里记了 detached_group_subscriptions,
	// 但那是事后。TODO:把那个计数接出去,在保存前弹确认。
	//

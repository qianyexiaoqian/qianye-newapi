...
| `TRUSTED_PROXIES` | Les en-têtes de transfert ne sont pris en compte que si le pair direct est lui-même un proxy approuvé, et `X-Forwarded-For` est dépilé de droite à gauche. Accepte des IP/CIDR ainsi que les mots-clés `none`, `loopback`, `private` et `cloudflare` (seul niveau qui accepte `CF-Connecting-IP`). Sans valeur : approuve le bouclage si `BIND_ADDRESS` est une adresse de bouclage, sinon n’approuve rien et avertit au démarrage | - |
| `CLIENT_IP_HEADERS` | En-têtes qu’un pair approuvé peut utiliser pour déclarer l’IP cliente, essayés dans l’ordre ; une liste explicite remplace entièrement la valeur par défaut. Les en-têtes propriétaires restent optionnels car un proxy inverse ordinaire retransmet tels quels les en-têtes fournis par le client | `X-Forwarded-For, X-Real-IP` |
| `TRUSTED_PROXIES_CLOUDFLARE_FILE` | Chemin vers une liste de plages Cloudflare (la sortie brute de `curl https://www.cloudflare.com/ips-v4`) ; elle remplace l’instantané intégré. Une liste contenant des préfixes privés, de bouclage ou trop larges interrompt le démarrage | - |
| `BIND_ADDRESS` | Adresse d’écoute ; vide écoute sur toutes les interfaces. Une adresse de bouclage fait aussi qu’un `TRUSTED_PROXIES` absent approuve les pairs de bouclage | - |

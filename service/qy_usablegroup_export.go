...
// 挂在更早的位置,上游会在 hook 之后再把 userGroup 塞回来,「一个用户分组可以不包含
// 它自己」就永远做不到 —— 本站那条 `-:自己` 的 GroupSpecialUsableGroup 规则从来没有
// 生效过,根因正是这一步(那套差分现已整体下线;自我补入仍在,只是收窄成
// 「名字必须是一个配了倍率的模型分组」,所以这个挂载点一个字都不能挪)。
//
// ─────────────────────── 逐位一致契约(实现方必须满足)───────────────────────
//
...
// ─────────────────────── 契约 ───────────────────────
//
// 返回 nil ⇒ 这一档**根本没有被接管**(没有 scope 行),调用方回落到全局
// options.AutoGroups —— 那是上游口径,与本功能无关。
//
// 空切片 ⇒ 这一档被接管了、但没有配 auto 顺序(或明确禁用了 auto)。
// 此时结果就是**没有 auto 分组**,不再回落全局:项目方拍板
// 「没有兜底这个说法,如果没有配置那就没有」。回落全局的实际后果也说不通 ——
// 全局清单里的分组未必在这一档的授权清单里,回落之后还要被逐个滤掉,
// 运营看到的是「我没配,它却按另一份我看不见的清单在转」。
//

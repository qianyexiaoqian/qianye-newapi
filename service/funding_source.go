...
// Settle 把结算差额落到钱包上:delta > 0 补收,delta < 0 退还。
//
// ═══════════ 拍过板的取舍:补收无条件成功,余额可以被扣成负数 ═══════════
//
// delta > 0 这一支走的是 model.DecreaseUserQuota,而它是一条裸的
// `UPDATE users SET quota = quota - ?`:**不校验余额、不夹 0、不会因为余额不够
// 而失败**。请求已经跑完、上游 token 已经烧掉,这一笔钱只能记上;唯一的问题是
// 记完之后余额是不是负的,而答案是——可以是负的,而且这是刻意的。
//
// 项目方原话(2026-08-19):
//
...
//	  性能和准确,高并发环境下只能先考虑性能。」
//
// 2026-08-10 还有更早的一次:一整轮"预扣改条件原子写 + 额度绕开批量队列"的改动
// 被**整条撤回**,因为它把有余额的用户 403 误拒了(实测第二笔报
// 「剩余 $0.006000, 需要 $0.054000」而用户真实余额 29972)。上游 issue #5690
// 报过同一批问题,维护者关成 NOT_PLANNED:「这是高并发下必须做的取舍,
// 如果你需要严格控制额度,请自行二开」。
//
// 代价写清楚,免得下一个人以为是白捡的:
//
//   - 余额会被扣成负数。8 路并发打一个只够 1 次的钱包 → 8/8 全过、余额 −140000。
//   - 欠款只能靠"用户回来充值"自动抵掉;**弃号跑掉的就是净损失**。
//   - 负余额账号的下一次请求会在预扣入口被拒(service/billing_session.go 的
//     wallet_overdrawn 档),文案与"余额不足"分开,写明欠多少。
//
// 配套的可运维手段(**只观测,不改计费**):
//
//   - 每一笔"预扣没兜住"的请求在消费日志里留 admin_info.pre_consume_shortfall。
//   - GET /api/qy/admin/overdraft:现在有多少账号是负的、合计欠多少、最深的是谁。
//
// **想改这段之前,先读 qianye/docs/decisions.md 的 D-01。** 那里记了是谁、
// 什么时候、为什么拍的板,以及上一次不知情地"修"它的人付出了什么。

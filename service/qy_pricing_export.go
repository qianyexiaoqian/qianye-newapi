...
// (用户分组, 模型分组) 的倍率矩阵表达。下面两个变量因此没有任何实现体给它们赋值,
// 全部保持恒等,结算结果与上游逐位一致。
//
// 它不是坏了,也不是死代码 —— 删它需要改 8 行上游计价代码(本包的 3 个调用点加
// relay/helper 的 5 个),而保留是 0 行。「接缝存在」由本包的
// qy_pricing_hookpoint_test.go 用 AST 钉死,「接缝空着」由
// qianye/pricing_seam_vacancy_guard_test.go 钉死。
//
// 下面这段记录的是「预扣与结算为什么会各按各的口径走」这个真实陷阱。它属于计费
// 链路本身,与任何一个扩展模块无关,重新启用分组级定价时原样适用,因此保留。
//
// ────────────────────────────────────────────────────────────────────────────
//
// 本文件是**结算侧**的挂载点。
...
//
// 「结算侧读 PriceData,所以覆盖三个计价点即覆盖全部扣费」这句话对 tiered 分支
// **不成立** —— 本文件的两个变量就是为这两条例外而存在的。
//

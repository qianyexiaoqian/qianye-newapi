 ...
 * 还留着的 `Link to='/qy/withdrawals'` 都要落到实处。目标 hash 由 `qyTabHash` 现算，
 * 与宿主页认标签用的是同一个函数 —— 不可能出现"跳过去了但选中的是另一张"。
 *
 * `replace`：旧地址不该留在历史栈里，否则用户按返回键会被立刻再弹回来。
 */
export const Route = createFileRoute('/_authenticated/qy/withdrawals/')({
  beforeLoad: () => {
    throw redirect({
      to: '/qy/affiliate',
      hash: qyTabHash('/qy/withdrawals'),
      replace: true,
    })
  },
})

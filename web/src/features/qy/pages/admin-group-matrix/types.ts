 ...
 * 一个用户分组的接管模式。
 *
 *   - `shadow`  已接管、但只记录不阻断。灰度的中间档。
 *   - `enforce` 清单是权威的：不在清单里的模型分组，令牌选不了、请求被 403。
 *
 * 「没有接管」不是第三个值而是 `managed:false` —— 行不存在 = 走上游原行为，
 * 与「接管了但清单是空的」必须能分开。后者合法且危险（隔离组），前者是零行为
 * 变更的默认态。用一个枚举表达三态会让这两件事再次不可区分。
 */
...
export type QyGmUserGroup = {
  ...
  /** 是否已被权威清单接管（`qy_group_scopes` 有没有这一行）。 */
  managed: boolean
  mode: QyGmMode
  /** 是否把 `auto` 伪分组注回可选清单。 */
  allow_auto: boolean
  ...
  note?: string
  /**
   * 这一行的接管**不是任何人按出来的**，而是「新分组默认全遮断」自动建的。
   *
   * 不渲染它，运营看到的就是一个莫名其妙已经 `enforce`、清单还空着的分组，
   * 而他确信自己没做过这件事 —— 那正是这个默认最容易被当成故障的时刻。
   * 它记录的是**来历**，配好清单之后也不会消失。
   */
  auto_masked: boolean
  /** 自动遮断发生的时间（unix 秒）。`0` = 未发生。 */
  auto_masked_at: number
  /**
   * 自动遮断了、而且到现在一个模型分组都没配。
   *
   * 与 `auto_masked` 分开是因为处置完全不同：来历要留着，待办要消失。
   * 合成一个字段会让提示在配置完成之后继续挂着，而长期挂着的提示等于没有提示。
   */
  pending_setup: boolean
  /**
   * 「新分组默认全遮断」开着的时候发现了这个分组，却**刻意没有遮断它**
   * （已经有人在用 / 那一轮冒出太多新分组）。
   *
   * 这一档比 `auto_masked` 更需要被看见：`auto_masked` 的落差是"发生了我没做过
   * 的事"，运营至少看得见那一行变成了 enforce；这一档的落差是"**没有**发生我以为
   * 会发生的事"，在界面上完全没有形状 —— 页面顶部写着「已开启」，这一行看起来
   * 和任何一个未接管的分组一模一样，而登记簿永不重判，意味着它此后也不会被
   * 补遮断。不渲染它，唯一的痕迹就只有扩展库里的一列 reason。
   */
  policy_skipped: boolean
  /** 当初的判断依据原话。空串表示没有记录。 */
  policy_skipped_reason: string
}

/**
 * 「我在别的页面新建一个用户分组，会发生什么」。
 *
 * 这段状态随每一次矩阵回读一起下发，而不是让前端去猜或再拉一个接口：它描述的是
 * 运营**下一次在上游「系统设置 → 分组倍率」页**加一个 key 时的后果，而唯一能让他
 * 在加之前看到这句话的地方就是这一页。
 */
export type QyGmNewGroupPolicy = {
  /** `group_matrix.new_group_default_deny`（且模块总开关也开着）。 */
  enabled: boolean
  /**
   * 发现延迟的上界（秒）。新分组不是立刻被遮断的 —— 上游没有「分组被创建」的
   * 事件可挂，扩展侧靠周期性对账发现它。运营加完分组盯着这一页看的时候，
   * 需要知道「还没变成遮断」是正常的，而不是坏了。
   */
  scan_interval_seconds: number
  /** 当前「自动遮断了、还一个模型分组都没配」的分组数。 */
  pending_setup: number
  /**
   * 当前「开关开着、发现过、却刻意没遮断」的分组数。
   *
   * 与 `pending_setup` 指向相反的两种落差，必须分开：前者是"被遮断了、等你配"，
   * 后者是"没被遮断，而你以为被遮断了"。
   */
  policy_skipped: number
  /**
   * 本节点最近一次对账完成的时间（unix 秒）。
   *
   * **`0` 不等于坏了**：对账走分布式租约，租约在别的节点手里时本节点恒为 0。
...
export type QyGmMatrixResponse = {
   ...
   *
   * 上游「系统设置 → 分组倍率」页改的是同一份数据且**刻意不锁死**（扩展关掉
  ...
  /** 「新分组默认全遮断」的当前状态。见 {@link QyGmNewGroupPolicy}。 */
  new_group_policy: QyGmNewGroupPolicy
}

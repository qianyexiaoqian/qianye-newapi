...
export type QyLotActivityDetail = {
   ...
   *
   * 玩法被隐藏后这一页**仍然可达** —— 已参与的人必须还能查到自己那一票、看结果
   * 与证据链，所以它不会变 404。但「参与」按钮要当场置灰并说明原因：留一个点下去
   * 才被 409 顶回来的按钮，与本仓一直在补的"界面上点得到、后端不认"是同一种缺陷。
   *
   * 老后端不下发时为 `undefined`，一律按**可参与**处理（真正说了算的是后端）。
   */
  play_open?: boolean

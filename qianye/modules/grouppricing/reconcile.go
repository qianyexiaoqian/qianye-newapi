...
//
// ⚠ **本文件对分组倍率免疫,不要跟着 effective.go 一起改。**
//
// effective.go 本轮从 GetGroupRatio(模型分组) 改成了
// service.GetUserGroupRatio(用户分组, 模型分组),因为它算的是**绝对价**,
// 而绝对价确实取决于用户分组。这里算的是**差额**:
//
//	差额 = 实际扣费 × (新值/旧值 − 1)
//
// 分组倍率同时出现在"实际扣费"和它自己里,在这个比值里**约掉了**。
// 于是无论 GroupGroupRatio 命中与否、命中的是对角格还是交叉格,
// 这个公式都精确成立 —— 它根本不需要知道用户分组是什么。
//
// 顺手"一起修"会把一个正确的公式改错:一旦在这里乘上某个用户分组的倍率,
// 分子分母就不再对称,算出来的差额会随着那个用户分组的倍率整体缩放,
// 而实际扣费一分没变。
//

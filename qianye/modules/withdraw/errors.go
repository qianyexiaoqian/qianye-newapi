...
var (
	...
	//
	// 文案里**不能**再建议"或驳回这张单":驳回只是把 frozen 搬回 available,
	// 欠账一位不动 —— 照着做的人会以为自己已经处理过了,而单据下一次还是被拦。
	// 正确的下一步只有一个:去佣金余额页把欠账清掉(结算会自动从可用余额里
	// 回收,回收不完的部分要走人工调账)。
	errDebtBlockedPayout = newBizError("qy_wd_debt_blocked_payout",
		"收款人存在冲正欠账,放款已被拦下。请先在佣金余额页对他跑一次结算把欠账清掉"+
			"(结算会从可用余额里自动回收);可用余额不够抵的部分需要人工调账。"+
			"驳回这张单不会减少欠账",
		http.StatusConflict)

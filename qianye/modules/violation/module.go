...
func (Mod) InstallHooks() {
	...
	// severity 列的清理。排在这里没有顺序要求(它没有任何读点),跟着其余
	// 一次性迁移走是为了让"启动期动过哪些表结构"只有一处可查。
	runLegacySeverityColumnDrop()

	// 全局审核抽样率的下线。这一条**必须排在下面那次预热之前**:预热会装配
	// AI 快照,而迁移出来的那条策略正是这个站点唯一能送审的一档 ——
	// 顺序反了的话,升级后的第一个刷新周期内 AI 审核是完全不跑的。
	//
	// 它与 severity 那一列不同,不是纯清理:先把值搬成一条作用域策略,再删列。

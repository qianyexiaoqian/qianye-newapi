...
// 调用方直接 return 即可。
//
// 形状刻意与同一个 handler 里已有的 `guard.RequireAPI(c, guard.Flag...)` 一致,
// 集成者只需在它下面再加一行。
//
...
// # 为什么没有"关闭支付密码校验"的开关
//
// 一个能关掉验密的配置项本身就是那条绕过路径。总闸是**调用方自己的功能开关**:
// 划转/提现/抽奖整体关掉时,这条路径根本不会被执行到,不需要第二个开关。
//
// 注意 verify 与本函数**不看任何 flag**,这是刻意的:验密是调用方入口已经
// 通过功能开关之后的事。但用户设置支付密码的那几个接口必须跟着"有没有哪条
// 路径会要它"走,而不是跟着划转走 —— 见 guard.FlagPayPassword 的说明,
// 那正是"关掉划转 = 静默关掉大部分人的提现"这个缺陷的形状。
//
// # ctx 为什么脱离请求
//

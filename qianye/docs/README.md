...
1. 所有新功能数据存进**独立数据库**,通过独立 YAML 配置(`./data/qianye.yaml`)。
...
### 拍过板的决定(**动计费/佣金/提现之前必读**)

[decisions.md](decisions.md) — 项目方拍过板的取舍:什么时候、谁拍的、为什么、
**代价是什么**。上面「已锁定的决策」那张表只有结论,这份文件有理由和代价。

看见一段代码"明显有问题"却先要读它,是因为吃过亏:2026-08-10 拍板接受的透支取舍
只活在一条 commit message 里,后来的人合理地把它当缺陷修了一遍,改完把有余额的
用户 403 误拒,只能整条撤回。**每一条都写了"可以改什么、不可以改什么"。**

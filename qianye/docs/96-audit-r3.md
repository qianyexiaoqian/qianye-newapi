...
| 3 | ~~**commission F1:`consumeEvent` 带上 `params.Group`,费率按计价分组解析**~~ → **口径由项目方裁定为「按账号分组」,本条关闭** | 见下方「口径裁决」。 |
...
F4(`effective`/`loadOverrides`/`refSalt` 接 ctx 并把查库移出临界区)、F5(降级告警 + `RiskFlags` 打标)、F6、F7、transfer #1/#3/#5、~~D3~~(已修:`guard.RecoverHot` 导出,`onRelaySample` 同步段挂上)、D4。
...
## 附:F1 的口径裁决(2026-07-31,项目方定)

**结论:分组费率按【下线的账号分组】(`users.group`)解析,四条来源统一。** 审计建议的
"按本次请求的计价分组"**不采纳**。

**审计原判的更正**:原文把 F1 写成"唯一可被用户主动构造的资损路径"。这一句过头了 ——
真正要占到差价,需要**上线与下线串通**:得有人先成为上线,再让下线把令牌指到低毛利分组。
单个用户自己做不到,他把自己令牌指到低分组只是自己少花钱、上线少拿佣金。定级应为
"合谋可利用",不是"可主动构造"。

**为什么选账号分组**:

- 讲得清。"这个下线是 VIP 客户,他的消费按 8% 返" 是运营能直接对用户讲的规则;
  "按你上一次请求实际落到哪个分组算" 讲不清,也不可预期。
- 四条来源(消费、充值、兑换码、任务补扣)共用一个档位。充值和兑换码根本不经过 relay,
  没有"计价分组"这个概念 —— 按计价分组解析必然导致消费一套档、充值另一套档。
- 日聚合桶按 `(下线, 日期, 费率)` 建键。按计价分组解析时,一个下线一天用了三个分组
  就裂成三行,"这个下线这天返了多少"不再是一个数,对账成本直接上去。

**已知代价与残留风险**:令牌可指定分组、auto 分组逐请求变化,所以计价分组可能 ≠ 账号分组。
上下线串通时能占到两者的费率差。**接受这个风险,不在扩展库另存一份计价分组** ——
主库 `logs` 表本身按请求记了实际计价分组,与 `qy_commission_accrual` 按 `(下线, 日期)`
join 即可比对,事后要查随时能查,不需要预先冗余一列。

**落地**:`consumeEvent` **不带**计价分组字段,让这条口径在编译期成立,而不是靠注释提醒。
回归:`TestAllSourcesUseAccountGroupForRate`(四条来源落库回读)+
`TestConsumeEventCarriesNoPricingGroup`(结构性)。三种破坏(消费丢分组、充值丢分组、
往事件塞回分组字段)均实测变红。

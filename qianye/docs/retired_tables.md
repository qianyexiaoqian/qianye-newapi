...
三张表在**那一轮**全部保留:表、代码、路由、`Tables()` 登记都不动,语义反转但字段不变。
那一轮退役的只有 `qy_group_seen` 一张,因为只有它是为自动接管而存在的。

(`qy_group_write_denies` 后来在 shadow 档下线的收尾里也退役了,见下一节。)
...
## groupmatrix 的「影子期写入拒绝」计数表

- **退役时间**:本轮(shadow 档下线的收尾)
- **观察期**:60 天
- **取代它的东西**:没有取代物 —— 这件事**不再发生**。

### 退役的是什么

shadow 是可用范围曾经的第三档:清单已配、但一个字节都不生效,用来在切 enforce
之前先看看会挡掉谁。这张表就是那段观察期里**唯一可归因**的证据 ——
`CheckTokenGroup` 在 `scope.Mode != ModeEnforce` 时不阻断、只累加一条
`(用户分组, 模型分组)` 计数。

shadow 已经整体下线(`hook.go`:现在只有「有 scope 行 = 清单立即生效」与
「没有 = 逐位返回上游」两档),存量 shadow 行由 `migrateShadowScopesToEnforce`
在启动时改写成 enforce。写入这张表的那条分支随之删除,于是它**再也不可能新增一行**。

| 表名                     | 退役时行数 | 说明                                             |
| ------------------------ | ---------- | ------------------------------------------------ |
| `qy_group_write_denies`  | 0(生产)  | 影子期「令牌写入侧本可拒绝」的 `(用户分组, 模型分组)` 折叠计数 |

行数是 0 的理由与上一节同源:写入它要求存在一条 `mode='shadow'` 的 scope 行,
而生产上 `qy_group_scopes` 从来是 0 行 —— 没有任何一档人进过影子期。

### 为什么连读侧一起删,而不是只删写侧

只删写侧、保留表与矩阵页那条横幅,看起来更保守,实际是把一句**已经不成立的话**
永久留在界面上:横幅的标题是「影子期记录到的令牌写入拒绝」,它的行动号召是
「切 enforce 之前先看证据」,而现在没有 enforce 可切 —— 勾完就生效。
运营看到它只会去找一个不存在的开关。

代价还不只是文案。`Tables()` 留着它,每一个**全新部署**都会被 AutoMigrate 建出这张
空表,而那台机器上从来没有存在过影子期;`residue.go` / `modelgroup_residue.go` 的
探测又刻意「0 行的残留仍然列出来」,于是每一次删用户分组、删模型分组的影响面预览上,
都会常驻一行「影子期本可拒绝计数 0 行」—— 一个恒真的 0,而它读起来像
"这个子系统还活着,只是这次正好没命中"。

历史行没有因此变得不可读:表还在库里,`/admin/health` 的 `retired_tables` 会报出它,
本文件解释它是什么。丢掉的只是「从矩阵页读它」这一条路径,而那条路径上顶着的
标题是错的。

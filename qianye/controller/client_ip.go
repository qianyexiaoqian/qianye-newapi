...
// # 为什么还要下发观测台
//
// 未配置 TRUSTED_PROXIES 时用的是上游那份默认(回环 + RFC1918 + fc00::/7),
// 它覆盖不到「反代在公网地址上」的部署(CDN 回源、独立 LB 主机)。那种站点
// 一切照常 200,只是所有人的 IP 都成了反代的地址。observations 就是这一档的
// 解药 —— 它记下「直连对端不受信、却带着转发头」的那些对端,并直接给出可以
// 粘进 TRUSTED_PROXIES 的 CIDR。把一个沉默的错配变成一条带着答案的诊断。
//
// 它是**提示**:这一页不改变任何结论,也不会让任何请求被拒。
//
// # 为什么是管理端
//

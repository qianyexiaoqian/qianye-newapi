...
var fieldConsumers = map[string]consumer{
	...
	"lottery.entry_batch_max_ms": {"qianye/modules/lottery/entry.go",
		"一次多注提交整批的时间预算上界(按部署方自己的反向代理读超时来配)"},
	...
	"lottery.max_guess_fee_bps": {"qianye/modules/lottery/settings.go",
		"手续费硬上界,防把 5% 手滑打成 50%"},
	"lottery.cover_enabled": {"qianye/modules/lottery/cover.go",
		"是否允许**上传**活动卡片背景图。关掉之后仍可填外链 —— 外链一个字节的磁盘都不占"},
	"lottery.cover_max_bytes": {"qianye/modules/lottery/cover.go",
		"单张封面的字节上限,魔数校验前的 MaxBytesReader 上界"},

...
//
// 为什么需要它:主库事务在 commit 阶段断连时会返回错误,但事务其实可能已经提交。
// Execute 现在按 COMMIT 有没有发出去分流(见 settleAfterMainFailure),那一支落
// StatusInDoubt 而不是 Failed,所以正向路径上的 Failed 已经是确定的了。
//
// 但 Failed 仍然不能被当作"主库没扣/没加钱"的**充分**证据,本函数因此保留:
//   - 升级前留下的存量 Failed 单混着两种含义,库里分不出来;
//   - 补偿任务与人工裁决也会落 Failed,那两条来自另一套判据;
//   - 一笔钱是否要被回滚/重发是不可逆动作,值得在动手前再问一次唯一的权威判据。
//
// 权威判据只有一个:与资金变更同事务写入的 outbox 行。
//

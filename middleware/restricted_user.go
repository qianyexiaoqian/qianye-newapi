...
var restrictedUserAllowedRoutes = map[string]struct{}{
	...
	// 受限账号公告:管理员配的那一段「你被限制了、该去哪申诉」。
	//
	// 不放开的话它对受限账号 403,而 403 在展示上与「没配置」不可区分 ——
	// 受限落地页会回落到那条固定文案,运营在后台配好的申诉渠道一个字都不显示,
	// 而且没有任何一处会报错。这正是本仓反复出现的「写了但没接上」:
	// 配置页能填、能存、能回读,线上永不生效。
	//
	// 只读、只对受限账号返回内容(handler 内再判一次 IsRestrictedUser),
	// 泄漏面为零 —— 它下发的是站点自己写给被限制用户看的公开文案。
	"GET /api/qy/restricted-notice": {},
